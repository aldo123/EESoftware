...
export default function LoginPage({ onLogin }) {
  ...
  return (
    <div className="min-h-screen bg-[#030712] flex items-center justify-center font-sans">
      <div className="w-[1320px] h-[700px] flex rounded-2xl overflow-hidden border border-[#1E293B] shadow-2xl">
        ...
        {/* RIGHT */}
        <div className="w-[400px] bg-[#0A0F1A] border-l border-[#1E293B] flex flex-col">
          ...
          <div className="flex flex-col items-center pt-2 pb-4 px-10">
            ...
            <p className="text-[#475569] text-xs tracking-widest uppercase font-mono">Technology · attuned to Nature</p>
            <div className="w-full h-px bg-[#1E293B] my-4" />
            <p className="text-[#94A3B8] text-sm font-medium">EE — Interlock & Traceability</p>
          </div>
          ...
          <div className="px-8 mb-1">
            <div className="flex bg-[#0F172A] rounded-xl p-1 border border-[#1E293B]">
              <button onClick={() => { setMode("card"); setError(""); }}
                className={`flex-1 py-2 rounded-lg text-xs font-semibold transition-all flex items-center justify-center gap-1.5 ${mode === "card" ? "bg-[#22C55E] text-[#052E16]" : "text-[#64748B] hover:text-[#94A3B8]"}`}>
                <IconCard /> Card Scan
              </button>
              <button onClick={() => { setMode("password"); setError(""); }}
                className={`flex-1 py-2 rounded-lg text-xs font-semibold transition-all flex items-center justify-center gap-1.5 ${mode === "password" ? "bg-[#22C55E] text-[#052E16]" : "text-[#64748B] hover:text-[#94A3B8]"}`}>
                <IconLock /> Password
              </button>
            </div>
          </div>
            ...
            {mode === "card" ? (
              ...
              <>
                ...
                <Field icon={<IconLock />} placeholder="Password" value={password} onChange={e => setPassword(e.target.value)} onKeyDown={e => e.key === "Enter" && doLogin()} type={showPass ? "text" : "password"}
                  suffix={<button type="button" onClick={() => setShowPass(p => !p)} className="text-[#475569] hover:text-[#94A3B8] transition-colors p-1"><IconEye open={showPass} /></button>} />
              </>
            )}

            {error && (
              <div className="flex items-center gap-2 bg-[#7F1D1D]/30 border border-[#EF4444]/30 rounded-lg px-3 py-2.5">
          ...
          <div className="px-8 pb-5 pt-4 mt-auto">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <span className="text-[#334155] text-xs font-mono">v1.0.0.1008</span>
                <button onClick={() => setShowSettings(true)}
                  className="w-7 h-7 rounded-lg flex items-center justify-center text-[#475569] hover:text-[#94A3B8] hover:bg-[#1E293B] transition-colors">
                  <IconSettings />
                </button>
              </div>
            </div>
